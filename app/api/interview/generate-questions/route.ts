...
`
    ...
    // If the client provided a custom openingTemplate, use it. Otherwise use the default multi-line template.
    const defaultOpeningTemplate = `本日はお忙しい中ご対応いただきありがとうございます。
...
それではさっそくインタビューに入らせていただきます。`
    ...
    const clientOpeningTemplate = (body as any).openingTemplate ? (body as any).openingTemplate : defaultOpeningTemplate
...
5. **インタビュー開始時の挨拶文 (openingMessage) は、以下のテンプレート（話す順）を必ず使用してください。[]の部分を具体的に埋め、テンプレートの行順・接続語（「今回は...ということで、」など）をそのまま反映してください。**：

${clientOpeningTemplate}

   ※[アカウント名]は、もし会社名等の情報があればそれを使用し、不明な場合は「BanKisha」等としてください。
   ※[目的]、[ターゲット]、[媒体]、[インタビュー名]は、前提条件の内容から抽出して埋めてください。
6. **出力は必ず純粋な JSON のみを返してください。他の言語説明、前置文、注釈、翻訳、あるいは余計な出力を一切含めないでください。**
7. コードブロックは使わないでください（バッククオート記号を含めないでください）。代わりにプレーンな JSON オブジェクトだけを応答として返してください。

【出力形式】
JSON形式で以下のキーを含めてください：
{
  "questions": ["質問1", "質問2", ...],
  "openingMessage": "インタビュー開始時の挨拶文（テンプレートに基づいたもの）",
...
`
    ...
    if (!parsed) {
      ...
      try {
        ...
        console.log('🧾 [RETRY] AI response preview:', retryText.substring(0, 500))

        parsed = extractAndParseJSON(retryText)
        if (!parsed) {
          throw new Error('Invalid AI response format even after retry')
        }
        ...
        console.error('❌ Question generation failed after retry:', retryError)
