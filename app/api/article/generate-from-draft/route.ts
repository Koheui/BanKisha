...
export async function POST(request: NextRequest) {
  try {
    ...
    // スキルナレッジベースから記事制作のベストプラクティスを取得（思考の起点として最優先）
...
内容の概要: ${section.contentOutline}`
    ...
    const prompt = `${skillKnowledgeContext ? `【最重要：思考の起点 - 記事制作のベストプラクティス（スキルナレッジベース）】\n${skillKnowledgeContext}\n\n**⚠️ 最重要**: 上記のスキルナレッジベースは、記事制作における思考の起点です。**必ず最初にこの内容を参照し、その原則と手法に基づいて記事を執筆してください。** このスキルナレッジベースに記載されている記事制作のベストプラクティス、ターゲット読者に響く書き方、メディア特性に合わせた記事構成の手法を**必ず実践**してください。\n\n` : ''}あなたは経験豊富なビジネス記事ライターです。以下の「敲き」（下書き/骨組み）を基に、指定された文字数で記事を執筆してください。
...
【記事制作の指示】
${skillKnowledgeContext ? `0. **最重要：スキルナレッジベースを思考の起点として活用**: 上記のスキルナレッジベースに記載されている記事制作のベストプラクティス、手法、原則を**必ず最初に参照**し、それに基づいて記事を執筆してください。スキルナレッジベースの内容を無視したり、軽視したりしないでください。\n` : ''}1. 敲きを忠実に反映: 提供された敲きの構成、見出し、要点、内容の概要を基に記事を執筆してください
...
4. 現在・過去・未来の順序: 必ず「現在」「過去」「未来」の順序で構成してください
...
${skillKnowledgeContext ? `9. **スキルナレッジベースの実践**: 上記のスキルナレッジベースに記載されている記事制作の手法、原則、ベストプラクティスを**必ず実践**してください。これらは思考の起点であり、記事制作の基盤です。\n` : ''}
...

{
  ...
  "sections": [
    {
      "heading": "見出し（敲きの見出しを基に）",
      "body": "本文（${sectionWordCounts[0]}文字程度、敲きの要点と内容の概要を具体的に展開）"
    },
    {
      "heading": "見出し（敲きの見出しを基に）",
      "body": "本文（${sectionWordCounts[1]}文字程度、敲きの要点と内容の概要を具体的に展開）"
    },
    {
      "heading": "見出し（敲きの見出しを基に）",
      "body": "本文（${sectionWordCounts[2]}文字程度、敲きの要点と内容の概要を具体的に展開）"
    }
  ],
  "totalWordCount": ${targetWordCount},
  "explanation": "${(draft.explanation || 'なぜこういう記事にしたのか？という解説').replace(/"/g, '\\"')}",
  "wordCountBreakdown": {
    "lead": ${Math.floor(targetWordCount * 0.1)},
    "現在": ${sectionWordCounts[0]},
    "過去": ${sectionWordCounts[1]},
    "未来": ${sectionWordCounts[2]}
  }
}
...
- 必ず「現在」「過去」「未来」の順序でセクションを配置してください
...
- JSONの構文エラーを避けるため、文字列内の特殊文字（改行、タブ、ダブルクォートなど）は必ずエスケープしてください`
    ...
    // セクションが3つあるか確認
    if (articleJson.sections.length !== 3) {
      return NextResponse.json(
        { error: '記事の生成に失敗しました。セクションは3つ（現在・過去・未来）である必要があります。' },
        { status: 500 }
      )
    }
